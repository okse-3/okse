...
public class ConfigController {
  ...
  public
  @ResponseBody
  ResponseEntity<String> addMapping(@RequestParam(value = "fromTopic") String topic,
      @RequestParam(value = "toTopic") String newTopic) {
    ...
    return new ResponseEntity<>(
        "{ \"message\" :\"Added mapping from Topic{" + topic + "} to Topic{ " + newTopic + " }\" }",
        HttpStatus.OK);
  }
  ...
  ResponseEntity<String> deleteMapping(@RequestParam(value = "topic") String topicToRemove) {
    ...
    return new ResponseEntity<>(
        "{ \"message\" :\"Deleted mapping for Topic{" + topicToRemove + "}\" }", HttpStatus.OK);
  }
  ...
  ResponseEntity<String> addRelay(
    ...
    log.debug(
        "Trying to add relay from: " + relay + " with topic:" + topic + ", using SOAP version "
            + version);
    ...
    if (host == null || port == null) {
      ...
      return new ResponseEntity<>(
          "{ \"message\" :\"Host or port not provided, not able to add relay\" }",
  ...
  public
  @ResponseBody
  ResponseEntity<String> deleteRelay(@RequestParam(value = "serverID") Integer id,
      @RequestParam(value = "relayID") String relay) {
    log.debug("Trying to remove a relay: " + relay);
    final WSNotificationServer server = wsnServers.get(id);

    if (server.deleteRelay(relay)) {
      return new ResponseEntity<>("{ \"message\" :\"Successfully removed the relay\" }",
          HttpStatus.OK);
    } else {
      return new ResponseEntity<>(
          "{ \"message\" :\"Unable to remove the relay, can't find it.\" }",
          HttpStatus.OK);
    }
  }
