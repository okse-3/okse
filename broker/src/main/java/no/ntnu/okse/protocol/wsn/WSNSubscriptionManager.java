...
public class WSNSubscriptionManager extends AbstractSubscriptionManager implements
  ...
  /**
   * This is the main OKSE implementation of the subscription manager addSubscriber method that
   * should be used. It will delegate core subscriber registry to the core SubscriptionService, as
   * well as update the local mappings from WS-Nu subscriptionKey to the relevant Subscriber and
   * SubscriptionHandle objects.
   *
   * @param s An instance of OKSE Subscriber with proper fields and attributes set.
   * @param subHandle An instance of WS-Nu SubscriptionHandle with proper fields and attributes
   * set.
   */
  public void addSubscriber(Subscriber s,
      AbstractNotificationProducer.SubscriptionHandle subHandle) {
  ...
  /**
   * Removes a Subscriber from the SubscriptionService, and the listener callback will remove it
   * from local mappings.
   *
  ...
  public AbstractNotificationProducer.SubscriptionHandle getSubscriptionHandle(String s) {
    if (localSubscriberHandle.containsKey(s)) {
  ...
  /**
   * Retrieve the SubscriptionHandle of the subscriber provided as the argument <p> This method
   * attempts to extract the WS-Nu subscriptionKey from the Subscriber object's attribute set, and
   * delegates the rest to the String based method with the same name.
   *
   * @param subscriber The Subscriber object to fetch the SubscriptionHandle of
   * @return A SubscriptionHandle connected to the Subscriber if it exists, null otherwise
   */
  public AbstractNotificationProducer.SubscriptionHandle getSubscriptionHandle(
   ...
   *
   * @param subscriptionReference The subscriptionReference to fetch related OKSE Subscriber object
   * from
  ...
  public Subscriber getSubscriber(String subscriptionReference) {
    if (localSubscriberMap.containsKey(subscriptionReference)) {
      return localSubscriberMap.get(subscriptionReference);
    }
    return null;
  }

  /**
   * This generic update method is intended to purge Subscribers that have a terminationTime set and
   * it has expired
   */
  @Override
  public void update() {
  ...
  (
    ...
    for (Map.Entry<String, String[]> entry : requestInformation.getParameters().entrySet()) {
      ...
      if (entry.getValue().length > 1) {
        ...
        if (localSubscriberMap.containsKey(subRef)) {
          ...
          log.debug(
              "Found subscriptionReference in local map, passing Subscriber object to SubscriptionService");
          _subscriptionService.removeSubscriber(localSubscriberMap.get(subRef));
          return new UnsubscribeResponse();
        } else {
          ExceptionUtilities
              .throwResourceUnknownFault("en", "Ill-formatted subscription-parameter");
        }
      } else if (entry.getValue().length == 0) {
        ExceptionUtilities.throwUnableToDestroySubscriptionFault("en",
            "Subscription-parameter in URL is missing value");
      }
      ...
      log.debug(
          "Found subscription reference in local map, passing Subscriber object to SubscriptionService");
    ...
    ExceptionUtilities.throwUnableToDestroySubscriptionFault("en",
        "The subscription was not found as any parameter" +
            " in the request-uri. Please send a request on the form: " +
            "\"http://urlofthis.domain/webservice/?" + WSN_SUBSCRIBER_TOKEN
   ...
   * @throws UnacceptableTerminationTimeFault If the terminationTime was either un-parsable or in
   * the past
   */
  @Override
  ...
  public RenewResponse renew(
    ...
    for (Map.Entry<String, String[]> entry : requestInformation.getParameters().entrySet()) {
      ...
      if (entry.getValue().length >= 1) {
        ...
        if (!localSubscriberMap.containsKey(subRef)) {
          ...
          ExceptionUtilities
              .throwResourceUnknownFault("en", "Given resource was unknown: " + subRef);
        }
      ...
      if (time < System.currentTimeMillis()) {
        ...
        ExceptionUtilities
            .throwUnacceptableTerminationTimeFault("en", "Tried to renew a subscription so it " +
    ...
    ExceptionUtilities
        .throwResourceUnknownFault("en", "The subscription was not found as any parameter" +
            " in the request-uri. Please send a request on the form: " +
            "\"http://urlofthis.domain/webservice/?" + WSN_SUBSCRIBER_TOKEN
            + "=subscriptionreference");
  ...
  public ResumeSubscriptionResponse resumeSubscription
      ...
      if (entry.getValue().length > 1) {
        ...
        if (!localSubscriberMap.containsKey(subRef)) {
          if (subscriptionIsPaused(subRef)) {
            log.debug(
                "SubscriptionReference in resume request found, passing to SubscriptionService");
        ...
        ExceptionUtilities
            .throwResumeFailedFault("en", "Subscription-parameter in URL is missing value");
      }
    ...
    ExceptionUtilities
        .throwResumeFailedFault("en", "The subscription was not found as any parameter" +
            " in the request-uri. Please send a request on the form: " +
            "\"http://urlofthis.domain/webservice/?" + WSN_SUBSCRIBER_TOKEN
            + "=subscriptionreference");
    return null;
  ...
  public PauseSubscriptionResponse pauseSubscription
    ...
    for (Map.Entry<String, String[]> entry : requestInformation.getParameters().entrySet()) {
      ...
      if (entry.getValue().length > 1) {
        ...
        ExceptionUtilities
            .throwPauseFailedFault("en", "Subscription-parameter in URL is missing value");
      }
    ...
    ExceptionUtilities
        .throwPauseFailedFault("en", "The subscription was not found as any parameter" +
            " in the request-uri. Please send a request on the form: " +
            "\"http://urlofthis.domain/webservice/?" + WSN_SUBSCRIBER_TOKEN
            + "=subscriptionreference");
